bands = struct('name', {'delta', 'theta', 'alpha', 'beta', 'gamma', 'gamma_high'}, ...
...
for ii = 1:length(percentages)
    ...
    for m = 1:M
        for n = 1:N
            ...
            datos_filtrados = bandpass(Potential{m, n}, band_values, Fs);%filtfilt(b, a, Potential{m, n}); 
    ...
    for m = 1:M
        for n = 1:N
            if ~isempty(Potential_damaged{m, n}) && all(Potential_damaged{m, n} ~= 0)
                datos_filtrados = bandpass(Potential_damaged{m, n}, band_values, Fs);%filtfilt(b, a, Potential_damaged{m, n});
                PDF = Bandt_y_Pompe_PDF_Original(datos_filtrados, tau, D);
                PDF = PDF / sum(PDF); % Normalize PDF
        ...
        for m = 1:M
            for n = 1:N
                if ~isempty(Potential_Restored{m, n}) && all(Potential_Restored{m, n} ~= 0)
                    datos_filtrados = bandpass(Potential_Restored{m, n}, band_values, Fs);%filtfilt(b, a, Potential_Restored{m, n});
   ...
   % text(plasticity_complexity, max_complexity, sprintf('%.2f', plasticity_complexity), 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right', 'FontSize', 10, 'Color', colors(ii,:)/255);
