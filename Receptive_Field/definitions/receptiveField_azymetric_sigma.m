function z = receptiveField_azymetric_sigma(x, y, A, x0, y0, sigma_x, sigma_y, sigma_x_old, sigma_y_old, theta)
...
%
    ...
    z_positive = A * exp(-(x_prime.^2/(2*sigma_x^2) + y_prime.^2/(2*sigma_y^2)));
    
    % Calculate the values of the receptive field for the negative deviations
    z_negative = A * exp(-(x_prime.^2/(2*sigma_x_old^2) + y_prime.^2/(2*sigma_y_old^2)));
