function [v1, v2, varargout] = ind2sub_rev(siz, ndx)
...
% Compute the reverse subscript indexing for rows
if nout > 2
    k = cumprod(siz);
    for i = nout:-1:3
...
if nout >= 2
    ...
    col = rem(ndx - 1, siz(2)) + 1;
    
    % Compute the row index
    row = (ndx - col) / siz(2) + 1;
    
    % Invert the row index for counting from bottom to top
    v1 = siz(1) - row + 1;
    v2 = col;
else
    v1 = double(ndx);
